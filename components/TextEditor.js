...
const TEXT = 'Type a Text';
const CLOSE_ICON = require('../utils/img/closed.png');
const COMPASS_ICON = require('../utils/img/compassed.png');

export default class TextEditor extends Component {
  constructor(props) {
    super(props);

    const {x, y, w, h, minWidth, minHeight} = props;

    this.state = {
      holders: [RIGHT_EDGE, RIGHT_HOLDER, LEFT_HOLDER, LEFT_EDGE, CENTER],
      x: x,
      y: y,
  ...
  childC = () => {
    return (
      <TouchableOpacity
        style={styles.center}
        onPress={() => this.hitCenter()}></TouchableOpacity>
    );
  };
  childTL = () => {
    return (
      <TouchableOpacity
        onPress={() =>
          this.props.TopLeftAction == undefined
            ? this.isBorder()
  ...
  childTR = () => {
    return (
      <TouchableOpacity
        onPress={() =>
          this.props.TopRightAction === undefined
            ? null
  ...
  onResizeMR = location => {
    const {isResizable, windowBorder, onResize} = this.props;
  ...
  onResizeML = location => {
    const {minWidth, isResizable, windowBorder, onResize} = this.props;
  ...
  onDrag = location => {
    const {isDraggable, windowBorder, onDrag} = this.props;
  ...
  render() {
    const {x, y, w, isBorder} = this.state;
    ...
    return (
      <View
        ...
        <TouchableOpacity
          ...
          <View
            style={{
              borderColor: isBorder ? 'transparent' : 'white',
              borderWidth: 1,
            }}
